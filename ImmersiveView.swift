...
struct ImmersiveView: View {
    ...
    func createQuestionBox() {
        ...
        if let tex = generatedStarTexture { particles.mainEmitter.image = tex }
    ...
    func createBrickBoxes() {
        ...
        let positions: [SIMD3<Float>] = [[-0.6, 1.5, -1.5], [ 0.6, 1.5, -1.5], [ 0.0, 2.1, -1.5]]
    ...
    func updateEffects(_ intensity: Float) {
        ...
        // ★熱対策: デバイスが熱い(Serious/Critical)ときは、パーティクル量を半分にする
        let performanceLimit: Float = thermalManager.isOverheating ? 0.5 : 1.0
        
        if let box = questionBoxEntity, var particles = box.components[ParticleEmitterComponent.self] {
            // intensity(手の動き) * 75(最大量) * performanceLimit(熱制限)
            particles.mainEmitter.birthRate = intensity * 75 * performanceLimit
...
extension GeometrySource {
    func asFloat3() -> [SIMD3<Float>] {
        (0..<self.count).map { self.buffer.contents().advanced(by: self.offset + self.stride * Int($0)).assumingMemoryBound(to: SIMD3<Float>.self).pointee }
    }
}
extension GeometryElement {
    func toUInt32Array() -> [UInt32] {
        let buffer = self.buffer.contents(); let count = self.count * 3
        if self.bytesPerIndex == 2 { let pointer = buffer.assumingMemoryBound(to: UInt16.self); return (0..<count).map { UInt32(pointer[Int($0)]) } }
        else { let pointer = buffer.assumingMemoryBound(to: UInt32.self); return (0..<count).map { pointer[Int($0)] } }
    }
}
